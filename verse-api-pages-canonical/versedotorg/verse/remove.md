...
Makes an `array` by removing `Input`'s elements from `StartIndex` to `StopIndex-1`. Succeeds if `0 <= StartIndex <= StopIndex <= Input.Length`.
|
---|---
Verse `using` statement | `using { /Verse.org/Verse }`
`(Input:[]t).Remove<public>(StartIndex:int, StopIndex:int where t:any)<computes><decides>:[]t`
## Parameters
