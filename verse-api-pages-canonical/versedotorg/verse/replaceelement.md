...
Makes an `array` by replacing the element at `IndexToReplace` with `ElementToReplaceWith` in `Input`. Succeeds if `0 <= IndexToReplace <= Input.Length-1`.
|
---|---
Verse `using` statement | `using { /Verse.org/Verse }`
`(Input:[]t).ReplaceElement<public>(IndexToReplace:int, ElementToReplaceWith:t where t:any)<computes><decides>:[]t`
## Parameters
